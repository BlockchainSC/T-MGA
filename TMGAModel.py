...
class TMGAModel(DetectModel):
    ...
    def compute_final_node_representations(self) -> tf.Tensor:      # TMP define
    ...
    def multihead_attention_readout(self, node_states):
        h = node_states                         # [V, H]
        heads = self.params['readout_num_heads']
        H = self.params['hidden_size']
        dk = H // heads

        with tf.variable_scope("mh_attn_readout"):
            # WK and WV projections
            ...
            # learnable global query per head (WQ-style)
            ...
            graph_emb = tf.reshape(graph_emb, [self.placeholders['num_graphs'], heads * dk])  # [G, heads*dk]
    ...
    def process_raw_graphs(self, raw_data: Sequence[Any], is_training_data: bool) -> Any:
        ...
        for d in raw_data:
            ...
            for nf in d["node_features"]:
                db.append(nf)
    #json file to model friendly format like temporal propagation schedule
